...
// SOURCE: https://betterprogramming.pub/singly-and-doubly-linked-lists-in-javascript-7515f47c9f60
...
class SinglyLinkedList {
  constructor() {
    this.head = null; // constructor tempat nyangkuting value berantai
    this.tail = null;
    this.length = 0;
  }
  ...
  push(val) {
    ...
    if (!this.head) {
      this.head = newNode; // Node { value: 1, next: null }
      console.log(this.head);
      this.tail = newNode; // Node { value: 1, next: null }
      console.log(this.tail);
      //otherwise add to end of list
    } else {
      console.log('this.tail before push: ', this.tail.next);
      this.tail.next = newNode; //Node { value: 1, next: Node { value: 2, next: null } }
      console.log("this.tail....: ", this.tail);
      this.tail = newNode;  //Node { value: 2, next: null }
      console.log("this.tail: ", this.tail);
    }

    console.log('currNode All: ', currNode);
    //add 1 to length
    this.length++;
    //return list
    return this //note: return this enabling us to create chain method
                //etc: LList.push(1).push(2).push(3).push(4);
  }
...
/*
push(val):  add value at tail
pop():  remove value at tail
unshift(val): add value at first head
shift():  remove value at first head
getNodeAtIndex(index):  
replaceNodeAtIndex(val, index): replacing/overwrite new value at index x
insertAtIndex(index, val):  insert new value at certain index, and move the following to next index;
removeFrom(index):
*/
...
// LList.unshift(2);
// LList.shift();
// LList.replaceNodeAtIndex(200, 1);
// LList.insertAtIndex(2, 300);
// LList.removeFrom(0)
// console.log('Node at index 1:', LList.getNodeAtIndex(1));

LList.printList();
// console.log('Total length:', LList.len());